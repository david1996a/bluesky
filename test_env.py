...
def main():
	for i in range(n_episodes):
		tic_e = time.time()
		state, observations = env.reset(i)
		scores = 0
		num_steps = 0
		done = False
		i_i = 0
		...
		while done == False:
			...
			next_state, next_observations, reward, dones, done = env.step()
			maddpg.step(i, state, observations, actions, reward, next_state, next_observations, dones)
			scores += np.min(reward)
			state = next_state
			num_steps += 1
		...
		print('Noise Scaling: {}, Memory size: {} and Num Steps: {}'.format(maddpg.maddpg_agents[0].noise_scale, 
			maddpg.memory.mem_cntr, num_steps))
